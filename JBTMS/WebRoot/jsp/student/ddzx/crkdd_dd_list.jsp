...
<html>
	<head>
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/crkdd.do?method=getDdList&page="+page;
			}
		}
		function check(ssxs){
			if(ssxs==0){
	...
	<body>
		<html:form action="/crkdd.do?method=queryDdList" method="post">
			...
			<table width="100%" align="center" cellpadding="0" cellspacing="1"
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							<td width="15%">
							<a href="<%=request.getContextPath()%>/crkdd.do?method=dDViewObject&pk=${obj.crkzj}" >${obj.ddbh}</a>
							</td>
							<td width="10%">
								${obj.ddlx }
							</td>
							...
							<td width="12%">
								<c:if test="${obj.ddzt eq 1 }">
									<a href="<%=request.getContextPath()%>/crkdd.do?method=goDd&pk=${obj.crkzj}" >调度</a> 
								</c:if>
