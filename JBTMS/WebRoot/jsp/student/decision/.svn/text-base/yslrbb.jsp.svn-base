  ...
  <head>
    ...
    <script type="text/javascript">
		function back(){
			var jlr = '${jlr}';
			if("jlr"==jlr){
				window.location.href="<%=request.getContextPath()%>/cwtj.do?method=ysJlrCwtj&kind=${kind}&ny=${ny}"
			}else{
				window.location.href="<%=request.getContextPath()%>/cwtj.do?method=ysLrCwtj&kind=${kind}&ny=${ny}"
			}
		}
		function toChild(){
    			...
    			<td colspan="4">
    				<table width="100%" align="center" cellpadding="0" cellspacing="1" border="0" style="word-break:break-all;">
    					...
    					<tr align="center">
    						...
    						<td>${vo.yf+vo.zf+vo.bf}</td>
    					</tr>
    					<c:if test="${'jlr' eq jlr }">
    						<tr align="center">
    						<td>1</td>
	    					...
	    					<tr align="center">
	    						<td></td>
	    						<td align="center">成本总计</td>
	    						<td>${vo.yfk+vo.ccyqfy }</td>
	    					</tr>
	    					<tr align="center">
	    						<td></td>
	    						<td align="center">利润总计</td>
	    						<td>${vo.yf+vo.zf+vo.bf-vo.yfk-vo.ccyqfy}</td>
	    					</tr>
    					</c:if>
    				</table>
    				</td>
