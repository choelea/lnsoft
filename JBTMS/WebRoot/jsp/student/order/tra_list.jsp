...
<html>
	<head>
    	...
    	function toadd(){
    		window.location.href="<%=request.getContextPath()%>/send.do?method=goSave&address=trans"
    	}
    	function todel(){
			...
			if(confirm("确认删除？")){
				...
				form.action="<%=request.getContextPath()%>/send.do?method=delete&address=trans";
		  		form.method="post";
		  		form.submit();
	  		}
    	}
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/send.do?method=getList&address=trans&page="+page;
			}
		}
	...
	<body>
		<html:form action="/send.do?method=query&address=trans" method="post" >
			...
			<table width="100%" align="center" cellpadding="0" cellspacing="1"
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							<td width="10%">
								<input type="checkbox" name="pk" value="${obj.ysdzj }"  <c:if test="${(obj.ssxs==0 and obj.sssysz==0) or obj.yszt!=0 }">disabled="disabled"</c:if>/>
							</td>
							<td >
								<a
									href="<%=request.getContextPath()%>/send.do?method=view&address=trans&pk=${obj.ysdzj}">${obj.ysdbh}</a>
							</td>
							<td>
							...
							<td >
								<c:if test="${obj.yszt==0 }">
									<a href="<%=request.getContextPath()%>/send.do?method=goUpdate&address=trans&pk=${obj.ysdzj}">修改
								</c:if>
							</td>
						</tr>
					</logic:iterate>
