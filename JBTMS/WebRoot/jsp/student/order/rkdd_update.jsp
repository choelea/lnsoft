...
<html>
  <head>
    ...
    <script type="text/javascript">
    	...
    	function pageAdata(curpage){
    		var countHql = "select count(*) from Khht A where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.htlx = ?";
			var listHql = "select new com.whjbsoft.www.vo.HtxxVo(A.htzj,A.htbh,A.htmc,B.ddzj,A.yfdj,A.bxfl,A.htlx) from Khht A inner join A.khdd B where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.htlx = ?";
    		UserDWR.getPageinfo(countHql,listHql,curpage,10,"仓储合同",function (data){
    	...
    	function pageBdata(curpage){
    		...
    		UserDWR.getPageinfo(countHql,listHql,curpage,10,str,function (data){
				...
				var msg = "<table border=\"0\" width=\"100%\" cellspacing='1' cellpadding='0' style=\"word-break:break-all;\">"+
		  				"<tr align=\"center\" height=\"30\">"+
					"<td>选择<input type='hidden' name='pagemsglx' value='kfxx'></td><td>库房编号</td><td>库房名称</td><td>库房类型</td></tr>";
		   		for(var obj in list){
		   			if(!isNaN(obj)){
						msg = msg +"<tr align=\"center\" height=\"25\"><td><input type=\"radio\" name=\"pk\" value=\""+obj+"\" />"+
  ...
  <script type="text/javascript" src="<%=request.getContextPath()%>/js/newdate.js"></script>
  	<html:form action="/crkdd.do?method=updateObject" method="post">
    		...
    		<caption><center>入库订单修改</center></caption>
    		<tr>
