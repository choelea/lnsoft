...
<html>
  <head>
    ...
    <script type="text/javascript">
    	...
    	function setValue(){
			...
			if(i!=-1){
				...
				if(pagemsglx.value=='rkdd'){					
					...
					var extkfzj = document.getElementsByName("extkfzj");
					var extkfmc = document.getElementsByName("extkfmc");
					...
					kfmc.value=extkfmc[i].value;
					kfzj.value=extkfzj[i].value;
    	...
    	function pageAdata(curpage){
    		var countHql = "select count(A) from Crkdd A inner join A.khht B  where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.ddzt=5 and A.ddlx = ?";
			var listHql = "select  new com.whjbsoft.www.vo.CrkddVo(A.crkzj,A.ddbh,A.khzlh,B.htzj,B.htmc,A.ddzt,A.sykfzj,A.kfmc,B.ssjg,A.crkfs) from Crkdd A inner join A.khht B where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.ddzt=5 and A.ddlx = ?";
  ...
  <script type="text/javascript" src="<%=request.getContextPath()%>/js/newdate.js"></script>
  	<html:form action="/crkdd.do?method=saveObject" method="post">
    		...
    		<caption><center>出库订单录入</center></caption>
    		<tr>
    			...
    			<td align="right" width="20%">入库订单&nbsp;&nbsp;<font color="red">*</font></td>
    			<td><input type="text" name="rkddbh" class="input" readonly="readonly"/>
