...
<html>
      ...
      function setValue(){		
			...
			if(i!=-1){
				...
				if(pagemsglx.value=='ysfslx'){					
					...
					getHt(ddzj[i].value);
					var khql = "select new com.whjbsoft.www.vo.KhddVo(A.ddzj,B.dwmc,B.lxr,B.lxrdh,B.ssjg,C.dwmc,C.lxr,C.lxrdh,C.ssjg,A.ddbh,A.ddrq,A.qhrq,A.ddzt,A.zdr) from Khdd A inner join A.khxxByKhzj B inner join A.khxxByKhxKhzj C where 1=1 and A.ddzj = " +ddzj[i].value;
		...
		function pageAdata(curpage){
			...
			var hql = "select new com.whjbsoft.www.vo.HtxxVo(A.htzj,A.htbh,A.htmc,B.ddzj,A.yfdj,A.bxfl,A.jzrq) from Khht A inner join A.khdd B where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.htzt=0 and (A.htzt=0 and A.htlx = ? or A.htlx='外包合同' and A.sfkyysps=1 and A.htzt=1)";
			UserDWR.getPageinfo(countHql, hql, curpage,10,"运输合同",cdc);
    	}
    	function cdc(data){
    		...
    		for(var obj in list){
    			if(!isNaN(obj)){
					msg = msg +"<tr align=\"center\" height=\"25\"><td><input type=\"radio\" name=\"pk\" value=\""+obj+"\" />"+
						  "<input type=\"hidden\" name=\"ls\" value=\""+list[obj].htbh+"\">"+
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/js/newdate.js"></script>
		<html:form action="/send.do?method=update&address=trans" method="post" onsubmit="return IsNull();">
			<table width="100%" align="center" cellpadding="0" cellspacing="1"
				border="0" style="word-break:break-all;">
				<caption><center>运输订单修改</center></caption>
				<tr>
				...
				<tr>
					...
					<td align="right">订单类型&nbsp;&nbsp;<font color="red">*</font></td>
					<td>
				...
				<tr>
					...
					<td><c:if test="${sendForm.vo.tbsm==0 }">
