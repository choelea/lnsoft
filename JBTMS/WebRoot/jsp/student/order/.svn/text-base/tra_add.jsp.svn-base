...
<html>
      ...
      function setValue(){		
			...
			if(i!=-1){
				...
				if(pagemsglx.value=='ysfslx'){					
					...
					var exthtlx = document.getElementsByName("exthtlx");
					var ddlx = document.all("ddlx");
					var ysdlx = document.all("vo.ysdlx");
					var ztj = $("ztj");
					ztj.value=0;
					var yfdj = $("khht.yfdj");
					var bxfl = $("khht.bxfl");
					...
					if(exthtlx[i].value=="运输合同"){
						ddlx.value="整车";
						ysdlx.value="运输";
					}else if(exthtlx[i].value=="配送合同"){
						ddlx.value="零担";
						ysdlx.value="配送";
					}
		...
		function pageAdata(curpage){
    		var countHql = "select count(*) from Khht A where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.htzt=0 ";
			var hql = "select new com.whjbsoft.www.vo.HtxxVo(A.htzj,A.htbh,A.htmc,B.ddzj,A.yfdj,A.bxfl,A.jzrq,A.htlx) from Khht A inner join A.khdd B where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.htzt=0";
					...
					msg = msg +"<tr align=\"center\" height=\"25\"><td><input type=\"radio\" name=\"pk\" value=\""+obj+"\" />"+
						  ...
						  "<input type=\"hidden\" name=\"ddid\" value=\""+list[obj].ddzj+"\">"+
						   "<input type=\"hidden\" name=\"yf\" value=\""+list[obj].yfdj+"\">"+
						    ...
						    "<input type=\"hidden\" name=\"exthtlx\" value=\""+list[obj].ssjg+"\">"+
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/js/newdate.js"></script>
		<html:form action="/send.do?method=save&address=trans" method="post" onsubmit="return IsNull();">
			<table width="100%" align="center" cellpadding="0" cellspacing="1"
				border="0" style="word-break:break-all;">
				<caption><center>运输订单录入</center></caption>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="text" name="ddlx" id="ddlx" class="input">
					</td>
