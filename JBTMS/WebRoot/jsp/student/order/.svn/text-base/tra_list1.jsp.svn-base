...
<html>
	<head>
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/send.do?method=objList&address=trans&page="+page;
			}
		}
	...
	<body>
		<html:form action="/send.do?method=queryList&address=trans" method="post" >
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							...
							<td >
								<a
									href="<%=request.getContextPath()%>/send.do?method=view1&address=trans&pk=${obj.ysdzj}">${obj.ysdbh}</a>
							</td>
							<td>
