...
<html>
	<head>
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/send.do?method=objList1&address=send&page="+page;
			}
		}
	...
	<body>
		<html:form action="/send.do?method=queryList1&address=send" method="post" >
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							...
							<td >
								<a
									href="<%=request.getContextPath()%>/send.do?method=view2&address=send&pk=${obj.ysdzj}">${obj.ysdbh}</a>
							</td>
							<td>${obj.ysfs }</td>
							<td>${obj.dhfmc }</td>
							<td>${obj.shfmc }</td>
							...
							<td >
								<c:if test="${obj.yszt==0 }">
									<a href="<%=request.getContextPath()%>/send.do?method=task&address=send&pk=${obj.ysdzj}">分 拣</a>
								</c:if>
							</td>
						</tr>
					</logic:iterate>
