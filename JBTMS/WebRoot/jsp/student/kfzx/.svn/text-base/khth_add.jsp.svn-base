...
<html>
  <head>
	...
	<script>
		...
		function getPagedata(curpage){
			var hql = " select new com.whjbsoft.www.vo.YspsdVo(A.ysdzj,A.ysdbh,A.ysdlx,A.ysfs,A.yszt,D.dwmc,E.dwmc,D.ssjg,E.ssjg) from Yspsd A inner join A.khht B inner join B.khdd C inner join C.khxxByKhzj D inner join C.khxxByKhxKhzj E where A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.yszt = 4 and (A.sfth != 1 or A.sfth is null)" ;
			var countHql = " select count(*) from Yspsd A inner join A.khht B inner join B.khdd C inner join C.khxxByKhzj D inner join C.khxxByKhxKhzj E where A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.yszt = 4 and (A.sfth != 1 or A.sfth is null)";
  ...
  <body>
    <html:form action="khth.do?method=save" method="post" onsubmit="return isNull();">
    	<table width="100%" border="0" cellpadding="0" cellspacing="1" style="word-break:break-all;">
    		<caption><center>客户退货录入</center></caption>
    		<tr>
    		...
    		<tr>
    			...
    			<td width="25%">
    				...
    				<html:hidden property="yspsd.ysdzj" value=""/>
