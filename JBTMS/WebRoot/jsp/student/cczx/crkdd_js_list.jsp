...
<html>
	<head>
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/cczx.do?method=getJsList&page="+page;
			}
		}
    </script>
		<c:if test="${msg!=null }">
	...
	<body>
		<html:form action="/cczx.do?method=queryJsList" method="post">
			...
			<table width="100%" align="center" cellpadding="0" cellspacing="1"
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							<td width="15%">
							<a href="<%=request.getContextPath()%>/cczx.do?method=viewObject&kind=js&pk=${obj.crkzj}">${obj.ddbh}</a>
							</td>
							<td width="10%">
								${obj.ddlx }
							</td>
							...
							<td width="12%">
								<c:if test="${obj.ddzt eq 2 }">
									<a href="<%=request.getContextPath()%>/cczx.do?method=goJs&pk=${obj.crkzj}" >接受</a> 
								</c:if>
