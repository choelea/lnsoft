...
<html>
  <head>
    ...
    <script type="text/javascript">
   	function back(){
   		var kind = '${kind}';
   		if("js"==kind){
   			window.location.href="<%=request.getContextPath()%>/cczx.do?method=getJsList&page=1";
   		}else if("zj"==kind){
  ...
  <body>
  	<html:form action="/cczx.do?method=updateObject" method="post">
  		<div id="printId">
    		...
    		<tr>
				...
				<c:if test="${ddzt eq 4 }">
					<c:set var="zt" value="出库" ></c:set>
				</c:if>
				<c:if test="${ddzt > 4 }">
					<c:set var="zt" value="质检" ></c:set>
				</c:if>
    			<td colspan="3"><input type="text" value="${zt }" class="input" readonly="readonly"/>
    				<html:hidden property="vo.ddzt" styleClass="input" />
    			</td>
    		</tr> 	
    		<tr>
    			<td align="right">库房信息&nbsp;&nbsp;&nbsp;&nbsp;
