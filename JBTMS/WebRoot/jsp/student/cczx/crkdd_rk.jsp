...
<html>
  <head>
    ...
    var i=0;//数组下标
   	...
   	function pageAdata(curpage){
   		...
   		var countHql = "select count(A) from Qwxx A inner join A.kfxx B  where 1=1 and B.ssxs = ${userPk} and B.sssysz = ${userSyszzj} and A.cclx=? and B.kfzj="+kfzj.value;
		var listHql = "select A from Qwxx A inner join A.kfxx B  where 1=1 and B.ssxs = ${userPk} and B.sssysz = ${userSyszzj} and A.cclx=? and B.kfzj="+kfzj.value;
   		UserDWR.getPageinfo(countHql,listHql,curpage,10,qwcclx,function (data){
			...
			if(cwlink!=null){
				...
				for(var c=0;c<arr.length;c++){
					if(arr[c].cwzj!=0){
						...
						var td1 = document.createElement("<td align='right'>");
						var text1 = document.createTextNode("储位:");
						...
						var text3 = document.createTextNode("上限:");
						td3.appendChild(text3);
						tr.appendChild(td3); 
						
						var td4 = document.createElement("<td>");
						var inp4 = document.createElement("<input type='text' name='cwsx"+djhpbm+"'readonly='readonly' class='inputsmall3'>");
						inp4.setAttribute("value",arr[c].cwsx);
						td4.appendChild(inp4);
						tr.appendChild(td4);
						
						var td5 = document.createElement("<td>");
						var text5 = document.createTextNode("已存:");
						td5.appendChild(text5);
						tr.appendChild(td5); 
						
						var td6 = document.createElement("<td>");
						var inp6 = document.createElement("<input type='text' name='ycsl"+djhpbm+"' readonly='readonly' class='inputsmall3'>");
						inp6.setAttribute("value",(parseFloat(arr[c].hgsl)+parseFloat(arr[c].shsl)));
						td6.appendChild(inp6);
						tr.appendChild(td6);
						
						var td7 = document.createElement("<td>");
						var text7 = document.createTextNode("合格:");
						...
						var text9 = document.createTextNode("损毁:");
						...
						var td11 = document.createElement("<td width='1%'>");//添加删除的td
						td11.innerHTML="<img src='<%=request.getContextPath()%>/images/button/del2.gif' alt='点击删除' style='cursor:hand;' onclick='removePut(this);'>";
	...
	function pageBdata(curpage){
  		var countHql = "select count(C) from Cwxx C inner join C.qwxx A inner join A.kfxx B  where 1=1 and B.ssxs = ${userPk} and B.sssysz = ${userSyszzj} and C.cwsx!=C.hgsl+C.shsl and C.hpbm=? and A.qwzj="+ccqwzj;
		var listHql = "select C from Cwxx C inner join C.qwxx A inner join A.kfxx B  where 1=1 and B.ssxs = ${userPk} and B.sssysz = ${userSyszzj} and C.cwsx!=C.hgsl+C.shsl and C.hpbm=? and A.qwzj="+ccqwzj;
	  	UserDWR.getPageinfo(countHql,listHql,curpage,10,djhpbm,function (data){
	    	var list = data.pagedata;
	   		var div1 = document.getElementById("div1");    		
			var msg = "<table border=\"0\" width=\"100%\" cellspacing='1' cellpadding='0' style=\"word-break:break-all;\">"+
	  				"<tr align=\"center\" height=\"30\">"+
				"<td>选择<input type='hidden' name='pagemsglx' value='cwxx'></td><td>储位编号</td><td>存储上限</td><td>已存数量</td></tr>";
	   		for(var obj in list){
	   			if(!isNaN(obj)){
					msg = msg +"<tr align=\"center\" height=\"25\"><td><input type=\"checkbox\" name=\"pk\" onclick='chosCw(this)' value=\""+obj+"\" />"+
						...
						"</td><td>"+list[obj].cwbh+"</td><td>"+list[obj].cwsx+"</td><td>"+(parseFloat(list[obj].hgsl)+parseFloat(list[obj].shsl))+"</td></tr >";					
	    		}
	   		}
	   		if(list.length==0){
	   			msg = msg + "<tr align=\"center\" height=\"30\"><td colspan=\"9\">暂无信息！</td></tr>";
	...
	function checkhgsl(temp){
		...
		for(var h=0 ; h<crhpbh.length; h++){
			var hgzsl = 0.0;
			var shzsl = 0.0;
			var cwsx = document.getElementsByName("cwsx"+crhpbh[h].value);
			var ycsl = document.getElementsByName("ycsl"+crhpbh[h].value);
			var hgsl = document.getElementsByName("hgsl"+crhpbh[h].value);		
			var shsl = document.getElementsByName("shsl"+crhpbh[h].value);
			for(var g=0;g<ycsl.length;g++){
				var cwccl = 0.0;
				if(hgsl[g].value!=""){
				...
				if(shsl[g].value!=""){
					if(!numVail.test(shsl[g].value)){
						...
						cwccl +=parseFloat(shsl[g].value);
					}
				}
				if(cwccl!=0.0){
					if(cwccl>cwsx[g].value-ycsl[g].value){
						alert("存放的货品数量大于储位上限！");
						return false;
					}
				}
			...
			if(parseFloat(hgzsl)>parseFloat(crhgsl[h].value)){
				alert("设置的存储合格数量已大于货品总合格数量！");
				return false;
			}
			if(parseFloat(shzsl)>parseFloat(crshsl[h].value)){
				alert("设置的存储损毁数量已大于货品总损毁数量！");
				return false;
			}
		}
	}
  ...
  <body>
  	<html:form action="/cczx.do?method=rkObject" method="post">
    		...
    		<tr>
    			...
    			<td colspan="3">
    				<input type="text" value="录入" class="input" readonly="readonly"/>
    		...
    		<tr>
    			<td align="center" colspan="4">
    				...
    				<input type="submit" value="入 库" class="inputbutton" onclick="return isNull();"/> 
