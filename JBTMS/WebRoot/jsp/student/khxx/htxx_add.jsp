  ...
  <head>
    ...
    <script type="text/javascript">
    	...
    	function IsNull(){
    		...
    		var ccdj = document.all("vo.ccdj");//存储单价
    		...
    		dForm.action="<%=request.getContextPath()%>/htxx.do?method=save";
    		dForm.method="post";
    		dForm.submit();
    		return true;
    	}
    	...
    	function setValue(){
			...
			if(i!=-1){
				...
				if(pagemsglx.value=='clis'){
					...
					var wtfyhxxs = document.getElementsByName("wtfyhxx");
					var wtfzhxxs = document.getElementsByName("wtfzhxx");
					var wtfssjgs = document.getElementsByName("wtfssjg");
					var wtf = document.all("vo.wtf");
					var lsid = document.getElementsByName("lsid");
					rkjs.value=ls[i].value;
					tyf.value=xzkhjl[i].value;
					wtf.value = wtfmc[i].value;
					khh.value=wtfyhxxs[i].value;
					khzh.value=wtfzhxxs[i].value;
					ssjg.value=wtfssjgs[i].value;
					getHt(lsid[i].value);
				}				
			}
		}
    	...
    	function pageCdata(curpage){
    		var hql = "select new com.whjbsoft.www.vo.KhddVo(A.ddzj,B.khzj,B.dwmc,B.lxr,B.lxrdh,B.ssjg,C.khzj,C.dwmc,C.lxr,C.lxrdh,C.ssjg,A.ddbh,B.khh,B.khzh) from Khdd A inner join A.khxxByKhzj B inner join A.khxxByKhxKhzj C where 1=1 and A.ssxs = ${userPk} and A.sssysz = ${userSyszzj} and A.ddzt = 1";
    		...
    		for(var obj in list){
    			if(!isNaN(obj)){
					msg = msg +"<tr align=\"center\" height=\"25\"><td><input type=\"radio\" name=\"pk\" value=\""+obj+"\" />"+
						  ...
						  "<input type=\"hidden\" name=\"wtfyhxx\" value=\""+list[obj].wtfyh+"\">"+
						  "<input type=\"hidden\" name=\"wtfzhxx\" value=\""+list[obj].wtfzh+"\">"+
						  "<input type=\"hidden\" name=\"wtfssjg\" value=\""+list[obj].wtfssjg+"\">"+
  ...
  <script type="text/javascript" src="<%=request.getContextPath()%>/js/newdate.js"></script>
  	<html:form action="/htxx.do?method=save" method="post">
    		...
    		<tr>
    			...
    			<td><html:text property="vo.tdr" styleClass="input" value="${userTName }" readonly="true"/></td>
