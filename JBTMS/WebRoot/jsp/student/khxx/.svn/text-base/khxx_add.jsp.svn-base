...
<html>
  <head>
    ...
    <script type="text/javascript">
    	...
    	function IsNull(){
    		...
    		dForm.action="<%=request.getContextPath()%>/khxx.do?method=save";
    		dForm.method="post";
    		dForm.submit();
    	}
    	function back(){
    	...
    	function valiValue(o,f){
    		if(o.value!=""){
    			var hql = " from Khxx where 1=1 and ssxs = ${userPk} and sssysz = ${userSyszzj} and dwmc = ?";
    			UserDWR.isExit(hql,o.value,function cd(data){
  ...
  <body>
  	<html:form action="/khxx.do?method=save" method="post">
  		  <table width="100%" align="center" cellpadding="0" cellspacing="1" border="0" style="word-break:break-all;">
    		<caption><center>客户录入</center></caption>
    		<tr>
    			<td align="right" width="20%">客户编号&nbsp;&nbsp;
    				<html:hidden property="vo.sftzwl" value="0"/>
    				<html:hidden property="vo.ssxs" value="${userPk }"/>
    				<html:hidden property="vo.sssysz" value="${userSyszzj }"/>
    			</td>
    			<td colspan="3">
    				<html:text property="vo.khbh" styleClass="input" readonly="true" value="${autobh }"/>
    				<div id="khbh"></div>
    			</td>
    		</tr>
    		<tr>
