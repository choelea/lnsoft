...
<html>
	<head>
		...
		function pagedata(page,count){
			...
			if(!exp.test(page)){
				...
				window.location.href="<%=request.getContextPath()%>/ysps.do?method=getList&address=qs&page="+page;
			}
		}
	...
	<body>
		<html:form action="/ysps.do?method=query&address=qs" method="post" >
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata">
						<tr align="center">
							...
							<td >
								<a
									href="<%=request.getContextPath()%>/ysps.do?method=view&vo.ysdzj=${obj.ysdzj}">${obj.ysdbh}</a>
							</td>
							<td >
							...
							<td >
								<c:if test="${obj.yszt==3 }">
									<a href="<%=request.getContextPath()%>/ysps.do?method=goQs&vo.ysdzj=${obj.ysdzj}">签 收</a>
								</c:if>
							</td>
						</tr>
					</logic:iterate>
