...
<html>
	...
	<body>
		<html:form action="/pgpc.do?method=queryFc" method="post">
				...
				<logic:notEmpty name="pageinfo" property="pagedata">
					<logic:iterate id="obj" name="pageinfo" property="pagedata"
						...
						<tr align="center">
							...
							<td>
								<a
									href="<%=request.getContextPath()%>/pgpc.do?method=view&vo.ysddzj=${obj.ysddzj}&type=fc">${obj.ysddbh}</a>
							</td>
							<td>
							...
							<td>
								<c:if test="${obj.ysddzt==1}">
									<a href="<%=request.getContextPath()%>/pgpc.do?method=ddfc&vo.ysddzj=${obj.ysddzj}" >发 车</a>
								</c:if>
								<c:if test="${obj.ysddzt>1}">
									已发车
								</c:if>
