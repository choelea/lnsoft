    ...
    <script type="text/javascript">
    	...
    	function IsNull(){
    		...
    		for(var i=0;i<divs.length;i++){
    			if(divs[i].id!=null && divs[i].id.indexOf("sjdf")!=-1){
	    			var lastE = divs[i].lastChild;
	    			while(lastE.nodeType!=1){
	    				lastE = lastE.previousSibling;
	    			}
	    			if(!expNum.test(lastE.value)){
	    				alert("评分有误，请输入有效数字!");
	    				return false;
	    			}
    			}
    		}
    		return confirm("是否确认阅卷?");
    	}
    </script>
  </head>
  
  <body>
    <html:form action="/djxxExam.do?m=update" method="post" onsubmit="return IsNull();">
    	<table width="100%" align="center" cellpadding="0" cellspacing="1" border="0" style="word-break:break-all;">
    		<tr align="center">
   				<td colspan="2">
   					...
   					<html:hidden property="vo.sfyj" value="1"/>
    			...
    			<c:set var="count" value="${tk.sjtx.zts+suffix}"/>
		    		...
		    		<logic:iterate id="st" name="tk" property="list" indexId="j">
		    			<tr>
		    				...
		    				<td align="center">
			    				<c:if test="${st[1].stlx=='单选题' || st[1].stlx=='多选题' || st[1].stlx=='判断题'}">	
				    				<span style="text-align: right;"><font color="red">得 分：${st[0].stda==st[1].zqda?(tk.sjtx.dtfs):0 }<br/>正确答案：${st[1].zqda }</font></span>
				    			</c:if>
		    			...
		    			<tr>
		    				...
		    				<html:hidden property="list[${suffix+j}].dtdf" value="${st[0].stda==st[1].zqda?(tk.sjtx.dtfs):0 }"/>
		    				</td>
		    			</tr>
		    			<tr>
		    				<td>
		    			...
		    			<tr>
		    				<td align="center" colspan="2">
		    				<div id="sjdf${j}">
		    				评 分：<html:text property="list[${suffix+j}].dtdf" styleClass="input" value="${st[0].dtdf }"/>
		    				</div>
		    				</td>
		    			</tr>
		    			</c:if>
    		...
    		<tr>
    			<td align="center" colspan="2">
    				<html:submit styleClass="inputbutton" value="阅 卷"></html:submit>
    				<input type="button" name="back" value="返 回" class="inputbutton" onclick="back1('./djxxExam.do?m=djxxList');"/>
    			</td>
    		</tr>
    	</table>
    </html:form>    
